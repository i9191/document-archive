...
const deleteLetter = () => {
    if (deleteId.value) {
        router.delete(route("letters.destroy", deleteId.value), {
            onFinish: () => {
...
<template>
    <AppLayout title="Arsip Surat">
        ...
        <div v-if="$page.props.flash?.success"
            class="mb-4 p-3 rounded bg-green-100 text-green-700 flex justify-between">
        ...
        <div class="p-6">
                ...
                <tbody>
                    <tr v-for="letter in letters" :key="letter.id" class="hover:bg-gray-50">
                        ...
                        <td class="border px-2 py-1 space-x-2">
                            <button @click="openDeleteModal(letter.id)" class="bg-red-500 text-white px-2 py-1 rounded">
                                Hapus
                            </button>
            ...
            <ConfirmDialog :show="showConfirm" title="Konfirmasi Hapus"
                message="Apakah Anda yakin ingin menghapus surat ini?" @close="showConfirm = false"
